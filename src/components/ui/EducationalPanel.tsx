...
export const EducationalPanel = () => {
  const [activeTab, setActiveTab] = useState<'theory' | 'code' | 'demo'>('theory');
  const { dispatch } = useSimulation();

  const loadCountToInfinityDemo = () => {
      // Create a linear topology A <-> B <-> C
      // But for Count to Infinity, usually A <-> B <-> C is enough where A becomes unreachable.
      // Let's create A --(1)-- B --(1)-- C
      // Then user can break A-B link.
